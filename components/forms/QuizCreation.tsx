...
const QuizCreation: React.FC<Props> = ({ topic: topicParam, files }: Props) => {
  ...
  const { mutate: getQuestions, isLoading } = useMutation({
    mutationFn: async ({ amount, topic, type, selectedFileId }: Input) => {
      const response = await axios.post("/api/game", { amount, topic, type, selectedFileId });
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Please provide the subject for the quiz.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div className="flex justify-between">
                <Button
                  variant={
                    form.getValues("type") === "mcq" ? "default" : "destructive"
                  }
                  className={`w-1/2 rounded-none rounded-l-lg ${mcq === true ? "text-green-500 font-extrabold" : ""}`}
                  onClick={() => {
                ...
                <Button
                  ...
                  className={`w-1/2 rounded-none rounded-l-lg ${oe == true ? "text-green-500 font-extrabold" : ''}`}
