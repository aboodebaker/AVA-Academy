...
const QuizCreation: React.FC<Props> = ({ topic: topicParam, files, id, subjects }: Props) => {
  ...
  const form = useForm<Input>({
    ...
    defaultValues: {
      topic: topicParam,
      subject: files.find(obj => obj.id === id)?.Subject.id,
      type: "mcq",
      amount: 3,
      selectedFileId: id,
    },
  ...
  useEffect(() => {
    form.setValue('selectedFileId', id)
  ...
  return (
       <div className='flex justify-center items-center full-screen md:w-[800] sm:w-[380] sm:m-3 '>
